...
type ParsedSimulationFile = {
    ...
    energy: {
        ...
        interfacial_energy: {
            type: string;
            parameters: { kappa: number };
        };
    };
    solvers: {
        Nstep: number;
        dt: number;
        solver: Array<{ type: string; M: number }>;
    };
    output: {
        writeEvery: number;
        logEvery: number;
...
class BoundaryConditions {
    ...
    constructor(configs: BoundaryConfig[]) {
        configs.forEach((cfg) => this.bcs.push(this.create(cfg)));
    }

    private create(cfg: BoundaryConfig): BoundaryCondition {
        if (cfg.type === "periodic") {
            return new PeriodicBC(cfg.position);
        }
        return new FixedBC(cfg.position, cfg.value ?? 0);
    }
    ...
    apply(mesh: Mesh): void {
        ...
        for (let j = 1; j <= mesh.Ny; j++) {
            for (let i = 1; i <= mesh.Nx; i++) {
                ...
                mesh.data[idx] = this.minVal + (this.maxVal - this.minVal) * Math.random();
            }
        }
    }
}
...
class Mesh {
    ...
    constructor(
        ...
        public readonly boundCons: BoundaryConditions,
        data?: number[],
    ) {
        this.invDxDy = 1 / (dx * dy);
        this.data = data ?? new Array((Nx + 2) * (Ny + 2)).fill(0);
    }

    outputMesh(kernel: MeshKernel): Mesh {
        const out = new Mesh(this.Nx, this.Ny, this.dx, this.dy, this.boundCons);
        ...
        out.boundCons.applyAll(out);
        return out;
    }
}

class DoubleWellBulk {
    constructor(public readonly A: number) { }

    dbulkDc(): (centerVal: number) => number {
        return (centerVal: number) =>
            2 * this.A * centerVal * (1 - centerVal) * (1 - 2 * centerVal);
    }
}

class GradientEnergy {
    constructor(public readonly kappa: number) { }
    ...
    dintDc(): MeshKernel {
        return (
            ...
            centerVal: number,
            invDxDy: number,
        ) => -this.kappa * invDxDy * (leftVal + rightVal + upVal + downVal - 4 * centerVal);
    }
}
...
class CHSolver {
    constructor(
        ...
        public readonly dbulkDc: (centerVal: number) => number,
        public readonly dintDc: MeshKernel,
        public readonly dt: number,
    ) { }

    step(): void {
        const dF = this.mesh.outputMesh(
            (
                leftVal: number,
                rightVal: number,
                upVal: number,
                ...
                centerVal: number,
                invDxDy: number,
            ) => this.dbulkDc(centerVal) + this.dintDc(leftVal, rightVal, upVal, downVal, centerVal, invDxDy),
        );

        const dc = dF.outputMesh(
            (
                leftVal: number,
                rightVal: number,
                upVal: number,
...
async function loadSimulationConfig(path: string): Promise<SimulationConfig> {
    ...
    const solver = parsed.solvers.solver.find((s) => s.type === "Cahn-Hilliard") ?? parsed.solvers.solver[0];
    if (!solver) {
        throw new Error("No Cahn-Hilliard solver found.");
    }

    const initType = mesh.initial_geometry.type;
    if (initType !== "random" && initType !== "random-uniform") {
        throw new Error("initial_geometry.type must be 'random' or 'random-uniform'.");
    }
    ...
    return {
        ...
        energy: {
            ...
            kappa: parsed.energy.interfacial_energy.parameters.kappa,
        },
        solver: {
            Nstep: parsed.solvers.Nstep,
            dt: parsed.solvers.dt,
            M: solver.M,
        },
        output: {
            writeEvery: Math.max(1, parsed.output.writeEvery),
            logEvery: Math.max(1, parsed.output.logEvery),
        },
    };
}
...
function App(): React.ReactElement {
    ...
    useEffect(() => {
        ...
        const runSim = async () => {
            try {
                ...
                const boundaries = new BoundaryConditions(config.mesh.boundaryConditions);
                const mesh = new Mesh(
                    config.mesh.Nx,
                    config.mesh.Ny,
                    config.mesh.dx,
                ...
                new UniformRandomInit(config.mesh.minVal, config.mesh.maxVal).apply(mesh);
                mesh.boundCons.applyAll(mesh);

                const bulk = new DoubleWellBulk(config.energy.A);
                const interfacial = new GradientEnergy(config.energy.kappa);
                const solver = new CHSolver(
                    mesh,
                    config.solver.M,
                    bulk.dbulkDc(),
                    interfacial.dintDc(),
                ...
                snapshots.push({
                    step: 0,
                    data: new Array(mesh.data.length).fill(0).map((_, i) => mesh.data[i]),
                });
                ...
                for (let istep = 1; istep <= config.solver.Nstep; istep++) {
                    solver.step();

                    if (istep % config.output.logEvery === 0) {
                        console.log(`Step: ${istep}`);
                    }

                    if (istep % config.output.writeEvery === 0) {
                        snapshots.push({
                            step: istep,
                            data: new Array(mesh.data.length).fill(0).map((_, i) => mesh.data[i]),
                        });
                ...
                setFrames(snapshots);
                setStatus(
                    `Completed in ${((endTime - startTime) / 1000).toFixed(2)}s. ${snapshots.length} frames.`,
                );
                setSelectedFrame(0);
            } catch (error) {
    ...
    useEffect(() => {
        ...
        const snapshot = frames[selectedFrame];
        ...
        const width = config.mesh.Nx;
        const height = config.mesh.Ny;
        canvas.width = width;
        canvas.height = height;

        const image = ctx.createImageData(width, height);

        // Render frame with coolwarm colormap [0.0, 1.0]
        for (let j = 0; j < height; j++) {
            for (let i = 0; i < width; i++) {
                const v = snapshot.data[(j + 1) * (width + 2) + i + 1];
                const t = Math.min(1, Math.max(0, v));
                const [r, g, b] = coolwarmMap(t);
                const p = (j * width + i) * 4;
                image.data[p] = r;
                image.data[p + 1] = g;
                image.data[p + 2] = b;
    ...
    const title = useMemo(() => "Spinodal Decomposition (Frame Browser)", []);
    const currentStep = frames.length > 0 ? frames[selectedFrame].step : 0;
