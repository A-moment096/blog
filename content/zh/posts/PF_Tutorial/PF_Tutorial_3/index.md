...
代码通常都有注释。C++ 中的单行注释以 `//` 开始，让编译器忽略一行中 `//` 后的所有内容。而多行注释（或者更准确地说，范围注释）则是以 `/*` 开始，以 `*/` 结束。简单更改程序时，单行注释很常用，而多行注释常用来书写大段说明性的文字，特别是版权信息等。
...
> [!QUESTION]
> 设现在有一个热源，其中心处在 $x = 64$ 的位置，宽度为$40$，温度为 $1$，再设整个模拟域的宽度为 $128$，且边界上采用固定边界条件，除了热源外的位置温度为 $1$。先在已知传热方程如下：
