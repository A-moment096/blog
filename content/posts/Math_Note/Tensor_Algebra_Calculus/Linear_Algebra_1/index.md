...
> 
> 记全体线性映射为 $\Hom_{\field{k}}(V,W)$ 或 $\mathcal{L}(V,W)$。若 $f$ 为双射且 $f^{-1}$ 亦线性，则称为线性同构。

总的来讲，$V$ 到 $W$ 的线性映射 $f$ 让下面的做法是完全可行的：我们可以直接将 $v\in V$ 映射到 $w = f(v) \in W$，也可以先将 $v$ 变为几个向量 $v_i$ 的线性组合，再把这些向量映射到 $w_i = f(v_i) \in W$ 中，最后再对它们进行线性组合。这两条路径将给出完全相同的结果。我们称从线性空间 $V$ 到其自身的线性映射为 *线性变换* 或者 *线性算子*，$V$ 上全体线性变换记为 $\End(V)$。
