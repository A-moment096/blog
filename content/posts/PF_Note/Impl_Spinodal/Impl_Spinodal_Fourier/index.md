...
没错，就是这样。接下来我们只需要用这个新的内积去替换原来的函数内积，就得到了离散傅里叶变换：
...
没错，只需要将函数内积替换为序列内积，我们就自然得到了离散傅里叶变换的结果。在我们不深入考虑傅里叶变换的离散与否对这个过程的影响的情况下，我们可以认为离散傅里叶变换是对传统傅里叶变换某种角度上的忠实反映。即在周期内对函数的取样结果进行离散傅里叶变换，与对函数进行傅里叶变换后再在区间上取样的结果，是相同的。也就是说，我们可以认为离散傅里叶变换能够忠实地反映函数傅里叶变换后的结果，因此我们可以认为它保留了傅里叶变换已有的诸多性质。

那要怎么具体实现呢？我们大可以就按照定义那样，从第一个点开始，一遍遍地做序列内积，得到每个 $k$ 的结果。但是您也许会注意到这样的做法会有很多遍的重复计算，又或者您早已听闻过 *快速傅里叶变换* 的大名。我们这里就干脆直接介绍什么是快速傅里叶变换算法，并简单实现一个，再与知名快速傅里叶变换算法库 *fftw* 做个小比较。
