...
这就是所谓的快速傅里叶变换 FFT 的动态循环算法，或者更规范地说，是 Cooley-Tukey 二分算法。它的优点在于不需要再依赖递归，这一点在计算大数据集的傅里叶变换时很重要，毕竟您也不想计算的过程中突然告诉你函数栈被塞爆了吧。我们自己实现的代码放在了这里：[复数类 C_my_fft_complex.h](/attachments/Impl_Spinodal/Fourier/C_my_fft_complex.h)，[测试用例 C_my_fft_test_main.c](/attachments/Impl_Spinodal/Fourier/C_my_fft_test_main.c)，[库源码 C_my_fft.c](/attachments/Impl_Spinodal/Fourier/C_my_fft.c)，[库头文件 C_my_fft.h](/attachments/Impl_Spinodal/Fourier/C_my_fft.h)。请自由下载编译使用。
